...
function App(){
	...
	function getJSON(json){ // getJSON
		...
		function event(){
			...
			.on("click","#layout .save_btn",onClickSaveBtn)
			.on("click","#layout .delete_btn",onClickDeleteBtn)
			.on("click","#layout .save-list>img",onClickSaveData)
			.on("click","#layout .save-list>span",onClickDeleteData)
		}
		// event
		function onClickSaveData(){
			let copy = savelist[$(this).data("idx")];
			data.type = copy.type;
			data.booth = Object.assign({},copy.booth);
			renderCanvas(TypeList[data.type]);
			renderDiv();
			renderSize();
			save();
		...
		function onClickDeleteData(){
			savelist.splice($(this).data("idx"),1);
			$(`#layout .save-list>*[data-idx="${$(this).data("idx")}"]`).remove();
			$("#layout .save-list>span").each( (idx,item)=>{
				$(item).attr("data-idx",idx).next().attr("data-idx",idx);
			})
			save();
		}
		function onClickSaveBtn(){
			savelist.push(Object.assign({},data));
			renderBooth(savelist[savelist.length-1]);
			renderCanvas(TypeList[data.type]);
			save();
		}
		...
		// render
		function renderBooth(item,idx=savelist.length-1){
			let array = JSON.parse(JSON.stringify(TypeList[item.type]));
			Object.entries(item.booth).filter( f => Object.entries(f[1]).length ).forEach( v =>{
				for(let miny = v[1].min.y,maxy = v[1].max.y ; miny <= maxy; miny++ ){
					for(let minx = v[1].min.x,maxx = v[1].max.x ; minx <= maxx; minx++ ){
						array[miny][minx] = v[0];
					}
				}
			});
			renderCanvas(array);
			renderText(item.booth);
		...
		function renderCanvas(array){
			...
			array.forEach( (y,yidx) =>{
				y.forEach( (x,xidx) =>{
					...
					if( x !== "block" && x !== "" ) draw(xidx*15,yidx*15,15,15,color[x]);
				});
			})
			ctx.beginPath();
		...
		function renderText(booth){
			Object.entries(booth).filter( f => Object.entries(f[1]).length ).forEach( (v,idx) =>{
				let x = v[1].min.x*15+(v[1].max.x-v[1].min.x+1)/2*15, 
				y = v[1].min.y*15+(v[1].max.y-v[1].min.y+1)/2*15;
				drawText(v[0],x,y);
			} )
		}
	...
	function drawText(text,x,y,color="white",font="맑은고딕",size=14){
		ctx.beginPath();
		ctx.texAlign = "center";
